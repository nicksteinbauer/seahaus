...
const DualAccessEnsuite = ({ stickyStays, mainStays }) => {
  ...
  return (
    <main className='page all'>
      ...
      <Head>
        <title>Dual Access Ensuite | SeaHaus Exuma Bahamas</title>
...
export async function getStaticProps() {
  const result = await fetch(`https://graphql.contentful.com/content/v1/spaces/${process.env.CONTENTFUL_SPACE_ID}/environments/master`, {
    ...
    body: JSON.stringify({
      query: `
        query {
          stickyStay:stayCollection(where: { category: "Dual Access Ensuite", categorySticky:true }, limit: 1) {
            items {
              title
              slug
              content {
...
export default DualAccessEnsuite