...
const BunkRoom = ({ stickyStays, mainStays }) => {

  return (
    <main className='page all'>
    
      <Head>
        <title>Bunk Room & Guest Room | SeaHaus Exuma Bahamas</title>
...
export async function getStaticProps() {
  const result = await fetch(`https://graphql.contentful.com/content/v1/spaces/${process.env.CONTENTFUL_SPACE_ID}/environments/master`, {
    ...
    body: JSON.stringify({
      query: `
        query {
          stickyStay:stayCollection(where: { category: "Bunk Room", categorySticky:true }, limit: 1) {
            items {
              title
              slug
              content {
          ...
          mainStay:stayCollection(where: { category: "Bunk Room", sticky:false}, order: roomOrder_ASC) {
            items {
              title
              slug
              category
